...
const sa = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../../service-account.json'), 'utf8'));
const app = initializeApp({ credential: cert(sa), projectId: 'dataaq-69662' });
const db = getFirestore(app);

async function runExhaustiveSeed() {
  console.log('🚀 EXHAUSTIVE SEEDING: Initializing all 28 collections in Cloud Firestore (dataaq-69662)...');
  const now = Date.now();
  ...
  const collections = [
    {
      name: 'users',
      data: [
        { id: 'usr-admin-01', uid: 'uid-admin-01', email: 'admin.national@bhumishield.gov.in', displayName: 'Dr. Rajesh Verma, IAS', phoneNumber: '+91-98201-11001', role: 'National Admin', isActive: true, createdAt: now, updatedAt: now },
        { id: 'usr-state-01', uid: 'uid-state-01', email: 'sec.revenue@maharashtra.gov.in', displayName: 'Shri A.K. Mehta, IAS', phoneNumber: '+91-98202-22002', role: 'State Admin', stateId: 'state-mh', isActive: true, createdAt: now, updatedAt: now },
        { id: 'usr-dist-01', uid: 'uid-dist-01', email: 'collector.palghar@gov.in', displayName: 'Dr. Manisha Patankar, IAS', phoneNumber: '+91-98203-33003', role: 'District Officer', stateId: 'state-mh', districtId: 'dist-palghar', isActive: true, createdAt: now, updatedAt: now },
        { id: 'usr-cala-01', uid: 'uid-cala-01', email: 'cala.palghar@gov.in', displayName: 'Shri Vikram Joshi, IAS', phoneNumber: '+91-98204-44004', role: 'Acquisition Officer', stateId: 'state-mh', districtId: 'dist-palghar', departmentId: 'dept-nhsrcl', isActive: true, createdAt: now, updatedAt: now },
        { id: 'usr-field-01', uid: 'uid-field-01', email: 'talathi.manikpur@gov.in', displayName: 'Shri Ramesh Sawant', phoneNumber: '+91-98205-55005', role: 'Field Officer', stateId: 'state-mh', districtId: 'dist-palghar', isActive: true, createdAt: now, updatedAt: now },
      ]
    },
    {
      name: 'roles',
  ...
  for (let i = 0; i < collections.length; i++) {
    ...
    console.log(`[${i + 1}/28] Seeding collection: '${col.name}' (${col.data.length} docs)...`);
    for (const doc of col.data) {
      await db.collection(col.name).doc(doc.id).set(doc);
      totalDocsWritten++;
    }
  }

  console.log(`\n🎉 SUCCESS! All 28 Collections (${totalDocsWritten} total documents) successfully written to Cloud Firestore (dataaq-69662)!`);
}

runExhaustiveSeed().then(() => process.exit(0)).catch(err => {
