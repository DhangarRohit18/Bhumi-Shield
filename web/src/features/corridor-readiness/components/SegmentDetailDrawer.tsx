...
export const SegmentDetailDrawer: React.FC<SegmentDetailDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-[1200] w-full max-w-xl bg-white shadow-2xl border-l border-[#BAE6FD] flex flex-col font-sans animate-in slide-in-from-right duration-300">
      {/* Header */}
      <div className="p-4 border-b border-[#BAE6FD]/80 bg-gradient-to-r from-[#F0F7FF] to-white flex items-start justify-between gap-3">
        <div className="space-y-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-black border ${statusConfig.badgeClass}`}>
              {segment.statusLabel}
            </span>
          ...
          <div className="flex items-center gap-2 text-xs text-[#0284C7] font-bold">
      ...
      {actionSuccessMessage && (
        <div className="mx-4 mt-3 p-2.5 bg-emerald-50 border border-emerald-300 rounded-xl text-xs font-bold text-emerald-800 flex items-center gap-2 shadow-sm animate-in fade-in">
      ...
      {/* Tabs */}
      <div className="flex border-b border-[#BAE6FD]/60 px-4 bg-[#F8FAFC]">
        {[
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {activeTab === 'OVERVIEW' && (
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-br from-[#F0F7FF] to-white border border-[#BAE6FD] shadow-xs flex items-center justify-between">
              <div>
              ...
              {segment.arWaypointCode && (
                <button
                  ...
                  className="px-3.5 py-2 rounded-xl bg-[#0F172A] text-white hover:bg-[#1E293B] transition-all flex items-center gap-2 cursor-pointer shadow-md text-xs font-extrabold group"
                >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              <div className="p-3 rounded-xl bg-[#F8FAFC] border border-slate-200">
                <span className="text-[10px] font-bold text-[#64748B] block">Parcels</span>
                <span className="text-base font-black text-[#0F172A] font-mono">{segment.affectedParcelsCount}</span>
              </div>
              <div className="p-3 rounded-xl bg-[#F8FAFC] border border-slate-200">
                <span className="text-[10px] font-bold text-[#64748B] block">Landholders</span>
                <span className="text-base font-black text-[#0F172A] font-mono">{segment.affectedLandholdersCount}</span>
              </div>
              <div className="p-3 rounded-xl bg-rose-50/70 border border-rose-200">
                <span className="text-[10px] font-bold text-rose-700 block">Objections</span>
                <span className="text-base font-black text-rose-700 font-mono">{segment.recordedObjectionsCount}</span>
              </div>
              <div className="p-3 rounded-xl bg-purple-50/70 border border-purple-200">
                <span className="text-[10px] font-bold text-purple-700 block">Court Stays</span>
                <span className="text-base font-black text-purple-700 font-mono">{segment.pendingLitigationCount}</span>
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-xl bg-rose-50/60 border border-rose-200 space-y-1.5">
              ...
              {segment.courtCaseNo && (
                <div className="pt-1.5 flex items-center justify-between text-[11px] font-mono font-bold text-purple-800 border-t border-rose-200/60 mt-1">
            ...
            <div className="p-4 rounded-xl bg-[#F8FAFC] border border-slate-200 space-y-2">
              <div className="flex justify-between items-center text-xs font-bold text-[#0F172A]">
                <span className="flex items-center gap-1.5">
                  <Coins className="w-4 h-4 text-[#EA580C]" /> Compensation & Solatium Disbursed
                </span>
            ...
            <div className="p-4 rounded-xl bg-white border border-[#BAE6FD] shadow-xs space-y-2.5">
              <h4 className="text-xs font-black text-[#0F172A] uppercase tracking-wider flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-[#EA580C]" /> Statutory Fast-Track Interventions
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <button
                  onClick={() => handleAction(`Fast-Track CALA Sec 15(1) Special Hearing for ${segment.villageName}`)}
                  className="px-3 py-2 rounded-lg bg-[#F0F7FF] hover:bg-[#E0F2FE] border border-[#BAE6FD] text-left text-xs font-bold text-[#0284C7] transition-all cursor-pointer flex items-center justify-between"
                >
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-lg bg-[#FAF5FF] hover:bg-[#F3E8FF] border border-[#E9D5FF] text-left text-xs font-bold text-[#7E22CE] transition-all cursor-pointer flex items-center justify-between"
                >
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-lg bg-[#FFF7ED] hover:bg-[#FFEDD5] border border-[#FFD8B5] text-left text-xs font-bold text-[#C2410C] transition-all cursor-pointer flex items-center justify-between"
                >
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-lg bg-[#ECFDF5] hover:bg-[#D1FAE5] border border-[#A7F3D0] text-left text-xs font-bold text-[#047857] transition-all cursor-pointer flex items-center justify-between"
                >
        ...
        {activeTab === 'DELAY_CHAIN' && (
          <div className="space-y-4">
            <div className="p-3 bg-[#F0F7FF] rounded-xl border border-[#BAE6FD] text-xs text-[#0369A1] font-medium leading-relaxed">
              ...
              {segment.delayChain?.map((step, idx) => {
                ...
                return (
                  <div key={idx} className="relative group">
                    ...
                    <div className="p-3.5 rounded-xl bg-white border border-slate-200 group-hover:border-[#BAE6FD] shadow-xs space-y-1.5 transition-all">
        ...
        {activeTab === 'TIMELINE' && (
          <div className="space-y-4">
            <div className="p-3 bg-[#F0F7FF] rounded-xl border border-[#BAE6FD] text-xs text-[#0369A1] font-medium">
              ...
              {segment.timeline?.map((item, idx) => (
                <div key={idx} className="p-3.5 rounded-xl bg-white border border-slate-200 space-y-1.5 shadow-xs">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-mono font-bold text-[#64748B] bg-slate-100 px-2 py-0.5 rounded">
                      {item.date}
                    </span>
                  </div>
                  <p className="text-xs text-[#475569]">{item.description}</p>
                  <div className="flex items-center justify-between text-[10px] text-[#0284C7] font-bold pt-1">
            ...
            {segment.evidenceDocuments?.map((doc, idx) => (
              <div key={idx} className="p-3.5 rounded-xl bg-white border border-slate-200 flex items-center justify-between gap-3 shadow-xs">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-xl bg-[#F0F7FF] text-[#0284C7] border border-[#BAE6FD]">
                    <FileText className="w-5 h-5" />
                  </div>
                  <div>
                    <h5 className="text-xs font-bold text-[#0F172A]">{doc.title}</h5>
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-slate-100 text-[#64748B] hover:text-[#0F172A] cursor-pointer"
                    title="View Document"
                  >
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-slate-100 text-[#0284C7] cursor-pointer"
                    title="Download"
                  >
